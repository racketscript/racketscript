...
(define (val->type-num v)
  (cond
    [(boolean? v)    0]
    [(char? v)       1]
    [(real? v)       2]
    [(symbol? v) (if (symbol-interned? v) 3.1 3)]
    [(keyword? v)    4]
    [(string? v)     5]
    [(bytes? v)      6]
    [(null? v)       7]
...
(define (hash-cmp x y)
  (define xtype (val->type-num x))
  (define ytype (val->type-num y))
  (if (= xtype ytype)
      ((val->cmp-fn x) x y)
      (< xtype ytype)))
...
(let ([u/apple (string->uninterned-symbol "apple")]
  ...
  (test (list #f #t
        ...
        'ordered
        ;; TODO: enable try-order? arg to hash-map
        ;; for now, sort after mapping
        (sort
         (hash-map (hash #t 'x
                         #f 'x
...
;; TODO: support extra fns in racket/hash, eg hash-union
...
(run-if-version "6.4.0.6" ;; for in-immutable-hash, etc
      ...
      (with-syntax 
       ...
       #'(define (name lst1 lst2)
          ...
          (define ht/weak (make-weak-hash (map cons lst1 lst2)))
           ...
           ;; TODO: implement chaperones and impersonators
  ...
  ;; racketscript (javascript) stack much smaller than Racket
  ;; (define lst3 (build-list 100000 values))
  ;; (define lst4 (build-list 100000 add1))
  (define lst3 (build-list 1000 values))
  (define lst4 (build-list 1000 add1))
...
(run-if-version "6.4.0.6" ;; for in-immutable-hash, etc
  ...
  ;; TODO: racketscript arithmetic-shift only works for 32 bit numbers
...
;;   (arity-test hash-ref-key 2 3)
  ...
  (define (test-hash-ref-key ht eql? expected-retained-key expected-excised-key)
    ...
    ;; orig tests (pass with racket, fail with racketscript):
    ;; (test #t eq? actual-retained-key expected-retained-key)
    ;; (test (eq? eql? eq?) eq? actual-retained-key expected-excised-key)
    ;; new tests (pass with racketscript; fail with racket)
    ;; (test #t eq? actual-retained-key expected-excised-key)
